...
func main() {
	...
	// Create a window.
	window, err := sdl.CreateWindow("Genetic Al-go-rithm", sdl.WINDOWPOS_UNDEFINED, sdl.WINDOWPOS_UNDEFINED, int32(winWidth), int32(winHeight), sdl.WINDOW_SHOWN)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	*/
	...
	aptR := ast.GetRandomNode()
	aptG := ast.GetRandomNode()
	aptB := ast.GetRandomNode()
	// How big to make it
	num := rand.Intn(20) // up to 20 nodes
	for i := 0; i < num; i++ {
		aptR.AddRandom(ast.GetRandomNode()) // no leaves yet
	}
	num = rand.Intn(20) // up to 20 nodes
	for i := 0; i < num; i++ {
		aptG.AddRandom(ast.GetRandomNode()) // no leaves yet
	}
	num = rand.Intn(20) // up to 20 nodes
	for i := 0; i < num; i++ {
		aptB.AddRandom(ast.GetRandomNode()) // no leaves yet
	}
	...
	for {
		...
		aptR.AddRandom(ast.GetRandomLeaf())
	}
	for {
		...
		aptG.AddRandom(ast.GetRandomLeaf())
	}
	for {
		...
		aptB.AddRandom(ast.GetRandomLeaf())
	}
...
func fbm2(x, y, frequency, lacunarity, gain float32, octaves int) float32 {
	...
	for i := 0; i < octaves; i++ {
		sum += snoise2.Snoise2(x*frequency, y*frequency) * amplitude // x * our frequency
...
func aptToTexture(redNode, greenNode, blueNode ast.Node, w, h int, renderer *sdl.Renderer) *sdl.Texture {
